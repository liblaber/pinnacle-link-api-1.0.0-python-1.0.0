...
class TeaserLinesResponseStatus(Enum):
    ...
    def list():
        ...
        """
        return list(
            map(lambda x: x.value, TeaserLinesResponseStatus._member_map_.values())
        )
...
class TeaserLinesResponseErrorCode(Enum):
    ...
    def list():
        ...
        """
        return list(
            map(lambda x: x.value, TeaserLinesResponseErrorCode._member_map_.values())
        )
...
class TeaserLinesResponse(BaseModel):
    ...
    def __init__(
        self,
        status: TeaserLinesResponseStatus,
        legs: List[TeaserLineLeg],
        error_code: TeaserLinesResponseErrorCode = None,
        price: float = None,
        min_risk_stake: float = None,
        max_risk_stake: float = None,
        min_win_stake: float = None,
        max_win_stake: float = None,
    ):
        self.status = self._enum_matching(
            status, TeaserLinesResponseStatus.list(), "status"
        )
        if error_code is not None:
            self.error_code = self._enum_matching(
                error_code, TeaserLinesResponseErrorCode.list(), "error_code"
            )
