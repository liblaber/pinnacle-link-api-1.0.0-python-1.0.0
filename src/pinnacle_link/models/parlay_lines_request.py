...
class ParlayLinesRequestOddsFormat(Enum):
    ...
    def list():
        ...
        """
        return list(
            map(lambda x: x.value, ParlayLinesRequestOddsFormat._member_map_.values())
        )
...
class ParlayLinesRequest(BaseModel):
    ...
    def __init__(
        self,
        odds_format: ParlayLinesRequestOddsFormat = None,
        legs: List[ParlayLineRequest] = None,
    ):
        if odds_format is not None:
            self.odds_format = self._enum_matching(
                odds_format, ParlayLinesRequestOddsFormat.list(), "odds_format"
            )
        if legs is not None:
