...
class ParlayLineLegErrorCode(Enum):
    ...
    def list():
        ...
        """
        return list(
            map(lambda x: x.value, ParlayLineLegErrorCode._member_map_.values())
        )
...
class ParlayLineLeg(BaseModel):
    ...
    :param error_code: When Status is PROCESSED_WITH_ERROR, provides a code indicating the specific problem.    CORRELATED - The leg is correlated with another one,     CANNOT_PARLAY_LIVE_GAME - The wager is placed on Live game,      EVENT_NO_LONGER_AVAILABLE_FOR_BETTING - The event is no longer offered for Parlays,     EVENT_NOT_OFFERED_FOR_PARLAY - The event is not offered for Parlays,     LINE_DOES_NOT_BELONG_TO_EVENT - LineId does not match the EventId specified in the request,     WAGER_TYPE_NO_LONGER_AVAILABLE_FOR_BETTING - Wager Type no longer available for betting,    WAGER_TYPE_NOT_VALID_FOR_PARLAY -  Wager Type not valid for parlay,     WAGER_TYPE_CONFLICTS_WITH_OTHER_LEG - Wager Type conflicts with other leg   SAME_EVENT_PERIODS_ARE_DISALLOWED - It's not allowed to parlay selected periods of the same event. , defaults to None
    ...
    def __init__(
        self,
        status: ParlayLineLegStatus,
        leg_id: str,
        error_code: ParlayLineLegErrorCode = None,
        line_id: int = None,
        ...
        if error_code is not None:
            self.error_code = self._enum_matching(
                error_code, ParlayLineLegErrorCode.list(), "error_code"
            )
        self.leg_id = leg_id
