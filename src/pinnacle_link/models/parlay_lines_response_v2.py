...
class ParlayLinesResponseV2Status(Enum):
    ...
    def list():
        ...
        """
        return list(
            map(lambda x: x.value, ParlayLinesResponseV2Status._member_map_.values())
        )
...
@JsonMap(
    {
        ...
        "max_parlay_risk_stake": "maxParlayRiskStake",
        "max_round_robin_total_risk": "maxRoundRobinTotalRisk",
        "max_round_robin_total_win": "maxRoundRobinTotalWin",
...
class ParlayLinesResponseV2(BaseModel):
    ...
    def __init__(
        self,
        status: ParlayLinesResponseV2Status,
        error: str = None,
        min_risk_stake: float = None,
        max_parlay_risk_stake: float = None,
        max_round_robin_total_risk: float = None,
        max_round_robin_total_win: float = None,
        round_robin_option_with_odds: List[RoundRobinOptionWithOddsV2] = None,
        legs: List[ParlayLineLeg] = None,
    ):
        self.status = self._enum_matching(
            status, ParlayLinesResponseV2Status.list(), "status"
        )
        ...
        if round_robin_option_with_odds is not None:
            self.round_robin_option_with_odds = self._define_list(
                round_robin_option_with_odds, RoundRobinOptionWithOddsV2
            )
        if legs is not None:
