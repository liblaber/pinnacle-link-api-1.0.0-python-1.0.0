...
class SpecialFixtureLiveStatus(Enum):
    ...
    def list():
        ...
        """
        return list(
            map(lambda x: x.value, SpecialFixtureLiveStatus._member_map_.values())
        )


@JsonMap(
    {"id_": "id", "bet_type": "betType", "date_": "date", "live_status": "liveStatus"}
)
class SpecialFixture(BaseModel):
    ...
    :param status: Status of the Special   O = This is the starting status. It means that the lines  are open for betting,   H = This status indicates that the lines are temporarily unavailable  for betting,   I = This status indicates that one or more lines have a red circle  (a lower maximum bet amount) , defaults to None
    ...
    def __init__(
        self,
        id_: int = None,
        bet_type: SpecialFixtureBetType = None,
        name: str = None,
        date_: str = None,
        cutoff: str = None,
        category: str = None,
        units: str = None,
        status: SpecialFixtureStatus = None,
        event: SpecialsFixturesEvent = None,
        contestants: List[SpecialsFixturesContestant] = None,
        live_status: SpecialFixtureLiveStatus = None,
    ):
        if id_ is not None:
            self.id_ = id_
        if bet_type is not None:
            self.bet_type = self._enum_matching(
                bet_type, SpecialFixtureBetType.list(), "bet_type"
            )
        if name is not None:
            self.name = name
        ...
        if status is not None:
            self.status = self._enum_matching(
                status, SpecialFixtureStatus.list(), "status"
            )
        if event is not None:
            self.event = self._define_object(event, SpecialsFixturesEvent)
        if contestants is not None:
            self.contestants = self._define_list(
                contestants, SpecialsFixturesContestant
            )
        if live_status is not None:
            self.live_status = self._enum_matching(
                live_status, SpecialFixtureLiveStatus.list(), "live_status"
            )
