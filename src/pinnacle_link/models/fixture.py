...
@JsonMap(
    {
        ...
        "alt_teaser": "altTeaser",
        "resulting_unit": "resultingUnit",
        "same_event_parlay_periods_enabled": "sameEventParlayPeriodsEnabled",
    }
)
class Fixture(BaseModel):
    ...
    :param live_status: Indicates live status of the event.  0 = No live betting will be offered on this event,  1 = Live betting event,  2 = Live betting will be offered on this match, but on a different event. Please note that [pre-game and live events are different](https://github.com/pinnacleapi/pinnacleapi-documentation/blob/master/FAQ.md#how-to-find-associated-events) . , defaults to None
    :type live_status: FixtureLiveStatus, optional
    :param status: This is deprecated parameter, please check period's `status` in the `/odds` endpoint to see if it's open for betting. O = This is the starting status of a game.   H = This status indicates that the lines are temporarily unavailable for betting,  I = This status indicates that one or more lines have a red circle (lower maximum bet amount). , defaults to None
    :type status: FixtureStatus, optional
    :param parlay_restriction: &nbsp;Parlay status of the event.  0 = Allowed to parlay, without restrictions,  1 = Not allowed to parlay this event,  2 = Allowed to parlay with the restrictions. You cannot have more than one leg from the same event in the parlay. All events with the same rotation number are treated as same event. , defaults to None
    ...
    :param resulting_unit: Specifies based on what the event will be resulted, e.g. Corners, Bookings  , defaults to None
    :type resulting_unit: str, optional
    :param version: Fixture version, goes up when there is a change in the fixture.  , defaults to None
    ...
    def __init__(
        ...
        away: str = None,
        rot_num: str = None,
        live_status: FixtureLiveStatus = None,
        status: FixtureStatus = None,
        parlay_restriction: ParlayRestriction = None,
        alt_teaser: bool = None,
        resulting_unit: str = None,
        version: float = None,
        same_event_parlay_periods_enabled: List[int] = None,
    ):
        if id_ is not None:
            self.id_ = id_
        ...
        if live_status is not None:
            self.live_status = self._enum_matching(
                live_status, FixtureLiveStatus.list(), "live_status"
            )
        if status is not None:
            self.status = self._enum_matching(status, FixtureStatus.list(), "status")
        if parlay_restriction is not None:
            self.parlay_restriction = self._enum_matching(
                parlay_restriction, ParlayRestriction.list(), "parlay_restriction"
            )
