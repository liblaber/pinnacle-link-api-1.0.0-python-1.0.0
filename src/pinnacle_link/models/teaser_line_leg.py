...
class TeaserLineLegErrorCode(Enum):
    ...
    def list():
        ...
        """
        return list(
            map(lambda x: x.value, TeaserLineLegErrorCode._member_map_.values())
        )
...
class TeaserLineLeg(BaseModel):
    ...
    :param error_code: When Status is PROCESSED_WITH_ERROR, provides a code indicating the specific problem.        EVENT_NOT_FOUND - The event specified could not be found,     POINTS_NO_LONGER_AVAILABLE - The points requested are no longer available. This means that the lines moved,      UNKNOWN - An unknown error has occured,     WAGER_TYPE_NOT_VALID_FOR_TEASER - The specified wager type is not valid for teasers     GAME_TEASER_DISABLED - Teasers are disabled for the event. , defaults to None
    ...
    def __init__(
        self,
        status: TeaserLineLegStatus,
        leg_id: str,
        error_code: TeaserLineLegErrorCode = None,
        line_id: int = None,
    ):
        self.status = self._enum_matching(status, TeaserLineLegStatus.list(), "status")
        if error_code is not None:
            self.error_code = self._enum_matching(
                error_code, TeaserLineLegErrorCode.list(), "error_code"
            )
        self.leg_id = leg_id
